...
function App() {
  ...
  async function createBudget() {
    ...
    if (!user) return;
    const { data: created, error } = await supabase.from("budgets").insert({
      name: "My Money",
      monthly_income: 3200,
      selected_month: currentMonth(),
      theme: "midnight",
      budgets: defaultBudgets,
      share_code: newBudgetCode(),
      owner_id: user.id
    }).select().single();

    if (error || !created) {
      alert(error?.message || "Could not create budget.");
      return;
    }

    await supabase.from("budget_members").insert({ budget_id: created.id, user_id: user.id, user_email: user.email, role: "owner" });
  ...
  if (!supabase) {
    return <main style={{ padding: 20, background: "#111", color: "white", minHeight: "100vh" }}><h1>Supabase is not connected</h1><p>Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in Vercel Environment Variables.</p></main>;
  }
      ...
      <main style={styles.page}>
        <section style={styles.phone}>
          <section style={styles.hero}>
            ...
            <p style={styles.muted}>Sign in with email so multiple people can update the same budget.</p>
            <label style={styles.label}>Email</label>
            <input style={styles.input} value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" />
            <button style={styles.primaryButton} onClick={sendMagicLink}>Send Login Link</button>
...
function makeStyles(theme) {
  return {
    ...
    installCard: { background: theme.panel, border: `1px solid ${theme.border}`, borderRadius: 22, padding: 14, marginTop: 14, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 },
